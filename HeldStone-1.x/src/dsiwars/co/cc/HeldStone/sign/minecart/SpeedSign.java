...
public class SpeedSign extends HeldSign {
	...
	protected boolean declare(boolean reload, SignChangeEvent event) {
		...
		try {
			this.speed = Double.parseDouble(this.getLines()[3]);
		} catch (NumberFormatException ex) {
			this.main.alert(getOwnerName(), "The entered speed is not a number", ChatColor.RED);
			if (!reload) {
				event.setCancelled(true);
			}
			return false;
		...
		if (speed <= 0.0D) {
			this.main.alert(getOwnerName(), "The entered speed must be greater than or equal to 0", ChatColor.RED);
			if (!reload) {
				event.setCancelled(true);
			}
			return false;
