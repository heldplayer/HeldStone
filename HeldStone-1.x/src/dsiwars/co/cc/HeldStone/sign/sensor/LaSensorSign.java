...
public class LaSensorSign extends HeldSign {
			...
			for (int x = xmin; x <= xmax; x++) {
				for (int y = ymin; y <= ymax; y++) {
					for (int z = zmin; z <= zmax; z++) {
						if (this.getWorld().getBlockAt(x, y, z).getTypeId() == 10 || this.getWorld().getBlockAt(x, y, z).getTypeId() == 11) {
							tripped = true;
							break CHECK;
						}
					}
	...
	protected boolean declare(boolean reload, SignChangeEvent event) {
		...
		if (!reload) {
			this.init("LaSensor sign accepted.");
		}
