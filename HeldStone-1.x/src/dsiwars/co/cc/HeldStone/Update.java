...
public class Update {
	...
	private static HeldStone plugin;

	public Update(HeldStone instance) {
		plugin = instance;
	}
	...
	public Boolean updateCheck() {
		try {
			...
			url.openConnection();
			new File(HeldStone.updatepath);
			double lastmodifiedurl = Double.parseDouble(getLatestVersion(plugin.versionaddress));
			double lastmodifiedfile = Double.parseDouble(HeldStone.version);
	...
	public void download() {
		...
		try {
			new File(HeldStone.updatepath);
			URL url = new URL(plugin.address);
			out = new BufferedOutputStream(new FileOutputStream(HeldStone.updatepath));
